...
data = pd.read_csv("All_Best_GPSCHYPE_MeanPrecision.csv")
print(data)
y = data.pop('MeanPrecision')
...
plt.title("Feature Importance Analysis on Mean Precision")
...
# plt.show()
# plt.savefig("FI_Mean_Precision.png",
#             dpi = 300,
#             bbox_inches = "tight")

df_test = pd.concat([FI_DF_1, feature_importance_df['importance'],
                     FI_DF_3['importance'],
                     FI_DF_4['importance']],axis=1)
df_test.columns = ['feature', 'importance Decision Tree Regressor',
                   'importance Random Forest Regressor',
                   'importance Extra Trees Regressor', 
                   'importance Gradeint Boosting Regressor']
df_test.to_csv("MeanPrecision_Feature importance.csv", index = False)
print(df_test)