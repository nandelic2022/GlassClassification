...
data = pd.read_csv("All_Best_GPSCHYPE_MeanRecall.csv")
print(data)
y = data.pop('MeanRecall')
...
plt.title("Feature Importance Analysis on Mean Recall")
...
# plt.show()
plt.savefig("FI_Mean_Recall.png",
            dpi = 300,
            bbox_inches = "tight")


df_test2 = pd.concat([FI_DF_1, feature_importance_df['importance'],
                      FI_DF_3['importance'],
                      FI_DF_4['importance']],axis=1)
df_test2.columns = ['feature', 'importance Decision Tree Regressor',
                    'importance Random Forest Regressor',
                    'importance Extra Trees Regressor', 
                    'importance Gradeint Boosting Regressor']
df_test2.to_csv("MeanRecall_Feature importance.csv", index = False)
# print(df_test)